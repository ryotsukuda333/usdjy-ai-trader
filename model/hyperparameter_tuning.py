...
def tune_regularization_only(df_features: pd.DataFrame, cv_folds: int = 3) -> Dict:
    """Tune regularization parameters using GridSearchCV with time-series split.

    Focuses on optimizing regularization parameters:
    - reg_alpha: [0.0, 0.5, 1.0]
    - reg_lambda: [0.0, 0.5, 1.0]
    - min_child_weight: [1, 3, 5]

    Keeps other hyperparameters fixed at optimal values from Step 2.2:
    - n_estimators: 100
    - max_depth: 4
    - learning_rate: 0.01
    - subsample: 0.6
    ...
    Total combinations: 27 (3×3×3) vs 3,888 full grid
    Much faster while focusing on regularization optimization.
    ...
    Returns:
        Dict with keys:
            - 'best_params': Best regularization parameters found
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print("REGULARIZATION PARAMETER TUNING - GRIDSEARCH")
        print(f"{'='*60}")
        print(f"Features: {len(X.columns)} | Samples: {len(X)} | Target distribution: {y.value_counts().to_dict()}")

        # Define parameter grid for regularization only
        # Fixed parameters based on Step 2.2 optimization
        param_grid = {
            'reg_alpha': [0.0, 0.5, 1.0],  # L1 regularization
            'reg_lambda': [0.0, 0.5, 1.0],  # L2 regularization
            'min_child_weight': [1, 3, 5],  # Minimum weight in leaf node
        }
        ...
        print(f"\n🔍 Grid Search Configuration (Regularization Focus):")
        ...
        print(f"  Estimated time: {total_combinations * cv_folds * 0.5 / 60:.1f} minutes (est.)")

        # Initialize base model with fixed optimal parameters from Step 2.2
        base_model = XGBClassifier(
            n_estimators=100,  # Fixed from Step 2.2
            max_depth=4,  # Fixed from Step 2.2
            learning_rate=0.01,  # Fixed from Step 2.2
            subsample=0.6,  # Fixed from Step 2.2
            colsample_bytree=0.8,  # Fixed from Step 2.2
        ...
        # GridSearchCV for regularization parameters only
        ...
        raise ModelError(
            ...
            user_message="Regularization parameter tuning failed",
...
def tune_hyperparameters(df_features: pd.DataFrame, cv_folds: int = 3) -> Dict:
    ...
    try:
        ...
        # Define parameter grid (Step 3: Added regularization parameters)
        param_grid = {
            ...
            'reg_alpha': [0.0, 0.5, 1.0],  # L1 regularization (new - Step 3)
            'reg_lambda': [0.0, 0.5, 1.0],  # L2 regularization (new - Step 3)
            'min_child_weight': [1, 3, 5],  # Minimum weight in leaf node (new - Step 3)
        }
...
def save_tuning_results(tuning_results: Dict) -> None:
    ...
    Handles both full hyperparameter tuning and regularization-only tuning.

    Args:
        tuning_results: Dictionary returned from tune_hyperparameters() or tune_regularization_only()
    ...
    try:
        ...
        # Determine if this is regularization-only tuning (has fewer unique param names)
        param_count = len(tuning_results['best_params'])
        is_regularization_only = param_count == 3  # Only reg_alpha, reg_lambda, min_child_weight

        # Save full grid results as CSV
        if is_regularization_only:
            grid_results_path = model_dir / 'regularization_tuning_results.csv'
        else:
        ...
        # Save improvement summary (if available from full tuning)
        if 'improvement' in tuning_results:
            ...
            improvement_path = model_dir / 'regularization_improvement.json'
            with open(improvement_path, 'w') as f:
                json.dump({
                    'regularization_params': params_dict,
                    'best_cv_score': float(tuning_results['best_score']),
                    'train_accuracy': float(tuning_results['train_accuracy']),
                    'train_f1': float(tuning_results['train_f1']),
                    'notes': 'Step 4 regularization-only tuning (fixed other hyperparameters)'
                }, f, indent=2)
            print(f"✓ Regularization improvement summary saved to {improvement_path}")
